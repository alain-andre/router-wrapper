...
class Wrappers::Osrm5Test < Minitest::Test
  ...
  def test_router_with_summed_by_area_true
    ...
    assert_equal [{way_type: 'urban', distance: 65.3}, {way_type: 'residential', distance: 407.9}, {way_type: 'interurban', distance: 418.1}, {way_type: 'secondary', distance: 75.5}], result[:features][0][:properties][:router][:summed_by_area], result[:features][0][:properties][:router][:summed_by_area]
    ...
    assert_equal [{way_type: 'urban', distance: 652.5}, {way_type: 'trunk', distance: 517.9}, {way_type: 'motorway', distance: 517.9}, {way_type: 'secondary', distance: 134.6}], result[:features][0][:properties][:router][:summed_by_area]
  ...
  def test_reverse_area_mapping
    osrm = RouterWrapper::OSRM5
    assert_equal %w[urban trunk], osrm.reverse_area_mapping(['l1'])
    assert_equal %w[urban_dense trunk], osrm.reverse_area_mapping(%w[l1 l2])
    assert_equal %w[water_body trunk], osrm.reverse_area_mapping(['l2'])
    assert_equal %w[interurban residential], osrm.reverse_area_mapping(['w1'])
    assert_equal ['interurban'], osrm.reverse_area_mapping(%w[w1 w2])
    assert_equal %w[interurban unassigned], osrm.reverse_area_mapping(%w[w1 w2 w3])
    assert_equal %w[interurban secondary], osrm.reverse_area_mapping(['w2'])
    assert_equal %w[interurban tertiary], osrm.reverse_area_mapping(%w[w2 w3])
    assert_equal %w[interurban primary], osrm.reverse_area_mapping(['w3'])
    assert_equal %w[interurban trunk], osrm.reverse_area_mapping([])
    assert_equal %w[interurban trunk motorway], osrm.reverse_area_mapping(['motorway'])
    assert_equal %w[urban trunk motorway], osrm.reverse_area_mapping(%w[l1 motorway])
    assert_equal %w[interurban unassigned motorway], osrm.reverse_area_mapping(%w[w1 w2 w3 motorway])
    assert_equal %w[urban secondary], osrm.reverse_area_mapping(%w[l1 w2])
  end
